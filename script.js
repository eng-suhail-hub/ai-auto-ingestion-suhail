...
const KEY_STORE = {
  pollinations: 'fv8_key_pollinations',
...
// ─────────────────────────────────────────
// CORS PROXY HELPER (PHP proxy on same Apache server)
// ─────────────────────────────────────────
/**
 * Uses proxy.php (served by the same XAMPP Apache) to forward API calls.
 * Since proxy.php is on the same origin as index.html, there are ZERO CORS issues.
 * Falls back to direct fetch if the PHP proxy is unavailable (e.g. Apache not running).
 */
const PHP_PROXY = 'proxy.php?url=';

async function fetchWithProxy(url, options = {}) {
  // Primary: PHP proxy on same Apache origin (no CORS)
  try {
    const proxyUrl = PHP_PROXY + encodeURIComponent(url);
    const r = await fetch(proxyUrl, options);

    // If the response is not OK, proactively check for HTML error pages
    if (!r.ok) {
      const clone = r.clone();
      try {
        const text = await clone.text();
        const trimmed = text.trim();
        if (trimmed.startsWith('<!DOCTYPE') || trimmed.toLowerCase().includes('<html') || trimmed.toLowerCase().includes('<head')) {
          log(`خطأ من الخادم (Apache): تم استلام صفحة HTML بدلاً من JSON.`, 'error');
          console.error('HTML Error Response:', text);
          throw new Error(`Server returned HTML error (check PHP logs). Status: ${r.status}`);
        }
      } catch (e) {
        if (e.message.includes('Server returned HTML')) throw e;
        // ignore other errors here, proceed to return r for provider logic
      }
    }

    // Only intercept errors that come from proxy.php itself.
    // proxy.php always returns {error: "string"}.
    // Provider API errors like OpenRouter return {error: {message, code}} (object) — let those pass through.
    const contentType = r.headers.get('content-type') || '';
    if (!r.ok && contentType.includes('application/json')) {
      const clone = r.clone();
      try {
        const errJson = await clone.json();
        // Only treat as proxy error if error is a plain string (proxy.php format)
        if (errJson.error && typeof errJson.error === 'string') {
          log(`خطأ داخلي من البروكسي: ${errJson.error}`, 'error');
          throw new Error(`Proxy Error: ${errJson.error}`);
        }
        // Otherwise it's a provider API error — fall through and return r to the caller
      } catch (e) {
        if (e.message.startsWith('Proxy Error:')) throw e;
      }
    }



    // Return the response object to let the calling function handle it
    return r;
  } catch (proxyErr) {
    if (proxyErr.message.includes('Proxy Error:') || proxyErr.message.includes('Server returned HTML')) throw proxyErr;
    log('البروكسي PHP غير متاح أو فشل — جاري المحاولة المباشرة...', 'warning');
    console.warn('Proxy failure:', proxyErr);
  }
  // Fallback: direct call (works for APIs that support CORS natively)
...
/**
 * Diagnostic tool to check if proxy.php is working and cURL is enabled.
 */
async function testProxy() {
  log('جاري اختبار البروكسي المحلي (proxy.php)...', 'sys');
  try {
    const res = await fetch('proxy.php?test=1');
    const text = await res.text();

    if (!res.ok) {
      log(`فشل الوصول للبروكسي ✗: HTTP ${res.status}`, 'error');
      alert(`Proxy Error (HTTP ${res.status}):\n\n${text.slice(0, 300)}`);
      throw new Error(`HTTP ${res.status}`);
    }

    try {
      const data = JSON.parse(text);
      log(`البروكسي يعمل ✅ (PHP ${data.php}, cURL ${data.curl})`, 'success');
      showNotification('البروكسي جاهز', `PHP ${data.php} — cURL ${data.curlOk ? 'مفعل' : 'معطل'}`, 'success');
    } catch (parseErr) {
      log('فشل تحليل رد البروكسي كـ JSON ✗', 'error');
      alert(`Invalid JSON from proxy:\n\n${text.slice(0, 300)}`);
      throw new Error('Not JSON');
    }
  } catch (e) {
    log(`خطأ: ${e.message}`, 'error');
    log('تأكد من وجود ملف proxy.php بجانب index.html وتفعيل cURL في XAMPP.', 'info');
  }
}
...
function saveConfig() {
  ...
  ['cfg_pol_model', 'cfg_pol_seed', 'cfg_or_model', 'cfg_groq_model',
    'cfg_gem_model', 'cfg_mis_model', 'cfg_hf_model', 'cfg_tog_model',
    'cfg_lang', 'cfg_prompt', 'cfg_json', 'cfg_path', 'cfg_naming', 'cfg_pattern',
    'cfg_concurrent', 'cfg_retries', 'cfg_bgMode', 'cfg_outRule'].forEach(id => {
...
// ─────────────────────────────────────────
const KEY_FIELDS = {
  pollinations: 'cfg_pol_key',
  openrouter: 'cfg_or_key',
  groq: 'cfg_groq_key',
  gemini: 'cfg_gem_key',
  mistral: 'cfg_mis_key',
  huggingface: 'cfg_hf_key',
  together: 'cfg_tog_key'
};
...
async function callAI(b64, onProgress) {
  switch (currentProvider) {
    ...
    case 'mistral': return await callMistral(b64, onProgress);
...
// ── 1. POLLINATIONS.AI (Free / Optional Key) ────────────────────────────────
async function callPollinations(b64, onProgress) {
  ...
  // مفتاح اختياري — يعمل بدونه (حد مجاني أقل)
  const API_KEY = getKey('pollinations');
  ...
  const response = await fetchWithProxy('https://gen.pollinations.ai/v1/chat/completions', {
  ...
  if (!response.ok) {
    ...
    try { const e = await response.json(); msg = e.error?.message || JSON.stringify(e); } catch (ex) { }
...
async function callOpenRouter(b64, onProgress) {
  ...
  if (!key) throw new Error('مفتاح OpenRouter API مطلوب — نماذج “مجانية” تتطلب حساباً ومفتاحاً من openrouter.ai/keys');
  const prompt = buildPrompt();
  const struct = buildStructureForAI();
  ...
  const response = await fetchWithProxy('https://openrouter.ai/api/v1/chat/completions', {
    method: 'POST', headers,
    body: JSON.stringify({
      model,
      messages: [{
  ...
  if (!response.ok) {
    ...
    try {
      ...
      console.error('[OpenRouter] Full error response:', JSON.stringify(r, null, 2));
      const err = r.error;
      if (err) {
        const providerRaw = err.metadata?.raw ? ` — خطأ المزود: ${String(err.metadata.raw).slice(0, 200)}` : '';
        e = (err.message || JSON.stringify(err)) + providerRaw;
      }
    } catch (ex) { }
    log(`OpenRouter Error (${response.status}): ${e}`, 'error');
...
async function callGemini(b64, onProgress) {
  ...
  onProgress(80, 'Gemini — معالجة...');
  if (!res.ok) {
    let msg = `HTTP ${res.status}`;
    try { const r = await res.json(); msg = r.error?.message || msg; } catch (ex) { }
...
async function callGroq(b64, onProgress) {
  ...
  const res = await fetchWithProxy('https://api.groq.com/openai/v1/chat/completions', {
    method: 'POST',
  ...
  if (!res.ok) {
    ...
    try { const r = await res.json(); e = r.error?.message || JSON.stringify(r); } catch (ex) { }
    throw new Error('Groq: ' + e);
  }
  const data = await res.json();
  return extractJSON(data.choices?.[0]?.message?.content || '');
}
...
async function callHuggingFace(b64, onProgress) {
  ...
  // HuggingFace Inference Router — supports all serverless models via one endpoint.
  // Model name goes in the request body (not the URL path).
  const hfUrl = 'https://router.huggingface.co/v1/chat/completions';

  const res = await fetchWithProxy(hfUrl, {
    method: 'POST',
    headers: { 'Authorization': `Bearer ${key}`, 'Content-Type': 'application/json' },
    body: JSON.stringify({
      model,
  ...
  try {
    ...
    if (text.toLowerCase().includes('<html')) {
      throw new Error(`HuggingFace: الخادم عاد بصفحة HTML (HTTP ${res.status}) — تحقق من اسم النموذج`);
    }
    throw new Error(`HuggingFace: رد غير صالح (HTTP ${res.status}): ${text.slice(0, 80)}`);
  }

  if (!res.ok) {
    // HF error can be a string or { message: '...' } object
    let em = data.error ?? data;
    if (typeof em === 'object') em = em.message || JSON.stringify(em);
    const emStr = String(em);
    if (emStr.toLowerCase().includes('loading')) {
      throw new Error('HuggingFace: النموذج يُحمَّل — انتظر 30 ثانية وأعد المحاولة');
    }
    if (res.status === 404) {
      throw new Error(`HuggingFace: النموذج غير موجود أو لا يدعم Chat Completions: ${model}`);
    }
    if (res.status === 403) {
      throw new Error('HuggingFace: مفتاح API غير صالح أو ليس لديك صلاحية الوصول لهذا النموذج');
    }
    throw new Error('HuggingFace: ' + emStr);
  }
  return extractJSON(data.choices?.[0]?.message?.content || '');
}
...
async function callTogether(b64, onProgress) {
  ...
  const imgUrl = b64.startsWith('data:') ? b64 : `data:image/jpeg;base64,${b64}`;
  const res = await fetchWithProxy('https://api.together.xyz/v1/chat/completions', {
    method: 'POST',
    headers: { 'Authorization': `Bearer ${key}`, 'Content-Type': 'application/json' },
    body: JSON.stringify({
      model, messages: [{
  ...
  if (!res.ok) {
    let e = `HTTP ${res.status}`;
    try { const r = await res.json(); e = r.error?.message || JSON.stringify(r); } catch (ex) { }
    throw new Error('Together: ' + e);
  }
  const data = await res.json();
  return extractJSON(data.choices?.[0]?.message?.content || '');
}

// ── 7. MISTRAL (Pixtral Vision — Free 1B tokens/month) ──────────────────────
async function callMistral(b64, onProgress) {
  const key = getKey('mistral');
  if (!key) throw new Error('مفتاح Mistral API مطلوب — احصل عليه مجاناً من console.mistral.ai');
  const model = document.getElementById('cfg_mis_model').value || 'pixtral-12b-2409';
  const prompt = buildPrompt();
  const struct = buildStructureForAI();
  onProgress(20, 'Mistral — إرسال...');

  const imgUrl = b64.startsWith('data:') ? b64 : `data:image/jpeg;base64,${b64}`;

  const res = await fetchWithProxy('https://api.mistral.ai/v1/chat/completions', {
    method: 'POST',
    headers: { 'Authorization': `Bearer ${key}`, 'Content-Type': 'application/json' },
    body: JSON.stringify({
      model,
  ...
  onProgress(80, 'Mistral — معالجة...');
  if (!res.ok) {
    let e = `HTTP ${res.status}`;
    try { const r = await res.json(); e = r.message || r.error?.message || JSON.stringify(r); } catch (ex) { }
    throw new Error('Mistral: ' + e);
  }
  const data = await res.json();
  return extractJSON(data.choices?.[0]?.message?.content || '');
}
